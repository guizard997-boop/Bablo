...
class Debt(db.Model):
    ...
    description = db.Column(db.Text, default='')
    status = db.Column(db.String(30), default='open')  # open / paid
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def admin_nav(active=''):
    def link(href, label, key):
        cls = 'font-bold text-brand' if active == key else 'text-gray-500'
        return f'<a href="{href}" class="{cls}">{label}</a>'
    return (
        '<div class="flex flex-wrap gap-3 text-sm mb-6">'
        + link('/admin/dashboard', 'Дашборд', 'dash')
        + link('/admin/products', 'Товары', 'products')
        + link('/admin/orders', 'Заявки', 'orders')
        + link('/admin/orgs', 'Организации', 'orgs')
        + link('/admin/debts', 'Долги', 'debts')
...
def admin_dashboard():
    ...
    org_c = Organization.query.count()
    open_debts = Debt.query.filter_by(status='open').all()
    debt_sum = sum(d.amount for d in open_debts)
    content = (
        '<div class="max-w-3xl mx-auto px-4 py-8">'
        + admin_nav('dash')
        + '<h1 class="text-xl font-extrabold mb-4">Дашборд</h1>'
        + f'<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">'
        + f'<div class="border rounded-2xl p-4"><div class="text-xs text-gray-400">Товары</div><div class="text-2xl font-extrabold">{pc}</div></div>'
        + f'<div class="border rounded-2xl p-4"><div class="text-xs text-gray-400">Заявки</div><div class="text-2xl font-extrabold">{oc}</div></div>'
        + f'<div class="border rounded-2xl p-4"><div class="text-xs text-gray-400">Организации</div><div class="text-2xl font-extrabold">{org_c}</div></div>'
        + f'<div class="border rounded-2xl p-4"><div class="text-xs text-gray-400">Открытые долги</div><div class="text-2xl font-extrabold text-red-500">{debt_sum:,.0f} сом</div></div>'
        + '</div>'
        + '<div class="flex flex-wrap gap-2">'
        + '<a href="/admin/debts/add" class="btn-o text-white font-bold px-4 py-2 rounded-xl text-sm">+ Добавить долг</a>'
        + '<a href="/admin/orgs/add" class="btn-g text-white font-bold px-4 py-2 rounded-xl text-sm">+ Организация</a>'
        + '</div></div>'
    )
...
def admin_orgs():
    ...
    rows = []
    for o in orgs:
        open_sum = sum(d.amount for d in o.debts if d.status == 'open')
        rows.append(
            f'<tr class="border-t"><td class="px-3 py-2 font-semibold">{o.name}</td>'
            f'<td class="px-3 py-2 text-sm text-gray-500">{o.phone or "—"}</td>'
            f'<td class="px-3 py-2 font-bold {"text-red-500" if open_sum else "text-gray-400"}">{open_sum:,.0f} сом</td>'
            f'<td class="px-3 py-2 text-sm">'
            f'<a href="/admin/debts?org={o.id}" class="text-brand">Долги</a> · '
            f'<a href="/admin/orgs/edit/{o.id}" class="text-brand">Изм.</a> · '
            f'<form action="/admin/orgs/delete/{o.id}" method="post" class="inline" onsubmit="return confirm(\'Удалить организацию и все долги?\')">'
    ...
    content = (
        ...
        + '<a href="/admin/orgs/add" class="btn-g text-white text-sm font-bold px-3 py-1.5 rounded-xl">+ Добавить</a></div>'
        + '<table class="w-full border rounded-xl overflow-hidden text-sm"><thead class="bg-gray-50 text-left">'
        + '<tr><th class="px-3 py-2">Название</th><th class="px-3 py-2">Телефон</th><th class="px-3 py-2">Долг</th><th class="px-3 py-2"></th></tr></thead><tbody>'
        + (''.join(rows) or '<tr><td colspan="4" class="p-6 text-center text-gray-400">Пока нет организаций</td></tr>')
        + '</tbody></table></div>'
    )
...
def admin_org_form(oid=None):
    ...
    content = (
        ...
        + admin_nav('orgs')
        + f'<h1 class="text-xl font-extrabold mb-4">{"Изменить" if o else "Новая"} организация</h1>'
        + '<form method="post" class="border rounded-2xl p-5 space-y-3">'
        + f'<input name="name" required value="{(o.name if o else "")}" placeholder="Название организации" class="w-full border rounded-xl px-4 py-2">'
        + f'<input name="phone" value="{(o.phone if o else "")}" placeholder="Телефон" class="w-full border rounded-xl px-4 py-2">'
        + f'<textarea name="note" rows="2" placeholder="Заметка" class="w-full border rounded-xl px-4 py-2">{(o.note if o else "")}</textarea>'
        + '<button class="btn-g text-white font-bold px-5 py-2 rounded-xl">Сохранить</button></form></div>'
    )
...
def admin_debts():
    ...
    if status_filter in ('open', 'paid'):
        q = q.filter_by(status=status_filter)
    debts = q.order_by(Debt.created_at.desc()).all()
    orgs = {o.id: o for o in Organization.query.all()}
    rows = []
    for d in debts:
        org_name = orgs[d.organization_id].name if d.organization_id in orgs else '—'
        st_cls = 'text-red-500' if d.status == 'open' else 'text-green-600'
        st_label = 'Открыт' if d.status == 'open' else 'Погашен'
        actions = ''
        if d.status == 'open':
            actions = f'<form action="/admin/debts/pay/{d.id}" method="post" class="inline"><button class="text-green-600 text-sm">Погасить</button></form> · '
        actions += f'<form action="/admin/debts/delete/{d.id}" method="post" class="inline" onsubmit="return confirm(\'Удалить?\')"><button class="text-red-400 text-sm">Удал.</button></form>'
        rows.append(
            f'<tr class="border-t"><td class="px-3 py-2 text-sm">{org_name}</td>'
            f'<td class="px-3 py-2 font-bold">{d.amount:,.0f}</td>'
            f'<td class="px-3 py-2 text-sm text-gray-500">{(d.description or "—")[:40]}</td>'
            f'<td class="px-3 py-2 text-sm {st_cls}">{st_label}</td>'
            f'<td class="px-3 py-2">{actions}</td></tr>'
        )
    total_open = sum(d.amount for d in Debt.query.filter_by(status='open').all())
    content = (
        '<div class="max-w-3xl mx-auto px-4 py-8">'
        + admin_nav('debts')
        + '<div class="flex flex-wrap justify-between items-center gap-2 mb-4">'
        + '<h1 class="text-xl font-extrabold">Долги</h1>'
        + '<a href="/admin/debts/add" class="btn-o text-white text-sm font-bold px-3 py-1.5 rounded-xl">+ Добавить долг</a></div>'
        + f'<p class="text-sm text-gray-500 mb-3">Открытых на сумму: <b class="text-red-500">{total_open:,.0f} сом</b></p>'
        + '<div class="flex gap-2 mb-4 text-sm">'
        + f'<a href="/admin/debts?status=open" class="px-3 py-1 rounded-full border {"bg-brand text-white" if status_filter=="open" else ""}">Открытые</a>'
        + f'<a href="/admin/debts?status=paid" class="px-3 py-1 rounded-full border {"bg-brand text-white" if status_filter=="paid" else ""}">Погашенные</a>'
        + f'<a href="/admin/debts?status=all" class="px-3 py-1 rounded-full border {"bg-brand text-white" if status_filter=="all" else ""}">Все</a>'
        + '</div>'
        + '<table class="w-full border rounded-xl overflow-hidden text-sm"><thead class="bg-gray-50 text-left">'
        + '<tr><th class="px-3 py-2">Организация</th><th class="px-3 py-2">Сумма</th><th class="px-3 py-2">Описание</th><th class="px-3 py-2">Статус</th><th></th></tr></thead><tbody>'
        + (''.join(rows) or '<tr><td colspan="5" class="p-6 text-center text-gray-400">Нет записей</td></tr>')
        + '</tbody></table></div>'
    )
...
def admin_debt_add():
    ...
    if request.method == 'POST':
        try:
            ...
            flash('Проверьте сумму и организацию', 'danger')
            return redirect(url_for('admin_debt_add'))
        if not org_id or amount <= 0:
            flash('Выберите организацию и укажите сумму', 'danger')
            return redirect(url_for('admin_debt_add'))
    ...
    if not orgs:
        content = (
            '<div class="max-w-md mx-auto px-4 py-8">' + admin_nav('debts')
            + '<p class="text-gray-500 mb-4">Сначала добавьте организацию.</p>'
            + '<a href="/admin/orgs/add" class="btn-g text-white font-bold px-4 py-2 rounded-xl">+ Организация</a></div>'
        )
        return page('Долг', content)
    content = (
        '<div class="max-w-md mx-auto px-4 py-8">'
        + admin_nav('debts')
        ...
        + f'<select name="organization_id" required class="w-full border rounded-xl px-4 py-2">{opts}</select>'
        + '<input name="amount" type="number" step="0.01" min="0.01" required placeholder="Сумма (сом)" class="w-full border rounded-xl px-4 py-2">'
        + '<textarea name="description" rows="2" placeholder="За что долг (необязательно)" class="w-full border rounded-xl px-4 py-2"></textarea>'
        + '<button class="btn-o text-white font-bold px-5 py-2 rounded-xl w-full">Сохранить долг</button></form></div>'
    )
...
# ---------- API синхронизации долгов (внешний сайт) ----------
def _api_key_ok():
...
def api_list_orgs():
    ...
    for o in orgs:
        open_sum = sum(d.amount for d in o.debts if d.status == 'open')
        out.append({'id': o.id, 'name': o.name, 'phone': o.phone or '', 'note': o.note or '', 'open_debt': open_sum})
...
def api_create_org():
    ...
    o = Organization(
        name=name[:200],
        phone=(data.get('phone') or request.form.get('phone') or '')[:50],
        note=(data.get('note') or request.form.get('note') or '')[:1000],
    )
    db.session.add(o)
    db.session.commit()
    return jsonify({'ok': True, 'id': o.id, 'name': o.name}), 201
...
def api_list_debts():
    ...
    debts = q.order_by(Debt.created_at.desc()).limit(500).all()
    items = []
    for d in debts:
        items.append({
            'id': d.id,
...
def api_create_debt():
    ...
    try:
        ...
        return jsonify({'ok': False, 'error': 'bad organization_id or amount'}), 400
    if not org_id or amount <= 0:
        return jsonify({'ok': False, 'error': 'organization_id and amount required'}), 400
    if not db.session.get(Organization, org_id):
...
def notify_new_order(order):
    """Уведомление админам о новом предзаказе с товарами."""
    ...
    try:
        ...
        for it in items:
            ...
            lines.append(f'• {pname} × {it.quantity} — {it.price * it.quantity:,.0f} сом')
    except Exception as e:
        print('order items error', e)
        lines.append('• (не удалось загрузить список)')
    lines.append('')
