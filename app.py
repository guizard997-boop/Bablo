...
def ai_describe_product(image_path):
    ...
    hf = os.environ.get('HF_TOKEN', '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').strip() or os.environ.get('HUGGINGFACE_TOKEN', '').strip()
    if hf:
        try:
...
def telegram_webhook():
    ...
    if photos:
        ...
        if name:
            ...
            db.session.commit()
            msg = '📷 Фото получено.'
            if not (os.environ.get('HF_TOKEN', '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') or os.environ.get('HUGGINGFACE_TOKEN') or os.environ.get('GEMINI_API_KEY') or OPENAI_KEY):
