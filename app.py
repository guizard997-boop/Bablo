...
def ai_describe_product(image_path):
    """Определяет товар по фото через Google Gemini."""
...
def telegram_webhook():
    ...
    if st.step == 'wait_price' and text:
        ...
        st.draft_price = price
        st.step = 'wait_category'
        db.session.commit()
        cats = Category.query.order_by(Category.name).all()
        if not cats:
            # нет категорий — сразу создаём
            product = Product(
                name=st.draft_name or 'Товар',
                description='',
                price=st.draft_price,
